...
class ModelCode extends CCodeModel
{
    ...
    public function attributeLabels()
    {
        return array_merge(
            ...
            array(
                ...
                'modelPath' => 'Путь до модели',
                'modelClass' => 'Класс модели',
                'baseClass' => 'Базовый класс',
                'buildRelations' => 'Построить зависимости',
                'commentsAsLabels' => 'Использовать коментарии к полям как ярлыки',
                'connectionId' => 'Соединение с базой',
            )
        );
    }
    ...
    public function prepare()
    {
        ...
        foreach ($tables as $table) {
            ...
            $params = array(
                // Шаблон кода в генератор модели
                'tpl' => file_get_contents($templatePath . '/tpl/model.php'),
                // Название таблицы
                ...
                'connectionId' => $this->connectionId, // Соединение
            );
            $this->files[] = new CCodeFile(Yii::getPathOfAlias(
                    $this->modelPath
                ) . '/' . $className . '.php', $this->render($templatePath . '/model.php', $params));
        }
    }
    ...
    public function validateBaseClass($attribute, $params)
    {
        ...
        if (!is_string($class) || !$this->classExists($class)) {
            $this->addError(
                'baseClass',
                "Class '{$this->baseClass}' does not exist or has syntax error."
            );
    ...
    {
        if ($this->tableName === $tableName || ($pos = strrpos($this->tableName, '.')) !== false && substr(
                $this->tableName,
                $pos + 1
            ) === $tableName
        ) {
    ...
    {
        if (Yii::app()->hasComponent($this->connectionId) === false || !(Yii::app()->getComponent(
                    $this->connectionId
